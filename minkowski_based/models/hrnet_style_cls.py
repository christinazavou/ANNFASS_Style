...
class HRNetStyleClsBase(Model):
    ...
    def __init__(self, in_channels, out_channels, config, D=3, **kwargs):
        ...
        super(HRNetStyleClsBase, self).__init__(in_channels, out_channels, config, D, **kwargs)
    ...
    def final_classification_layers(self, D, backbone_out_feat, bn_momentum, out_channels):
        self.emb_cp = MinkowskiPoolFeaturesPerComponent(backbone_out_feat)
        self.fc1 = conv(in_planes=backbone_out_feat, out_planes=self.CLASS_LAYER_DIM, kernel_size=1, stride=1, bias=True,
                   D=D)
        self.bnfc1 = get_norm(norm_type=self.NORM_TYPE, n_channels=self.CLASS_LAYER_DIM, D=D, bn_momentum=bn_momentum)
        self.fc2 = conv(in_planes=self.CLASS_LAYER_DIM, out_planes=out_channels, kernel_size=1, stride=1, bias=True, D=D)
    ...
    def load_from_ps(self, ps_checkpoint):
        if os.path.exists(ps_checkpoint):
            ...
            for name, param in self.named_parameters():
                if any(name.startswith(final_layer) for final_layer in ['fc1', 'bnfc1', 'fc2']):
    ...
    def forward(self, x: SparseTensor, components_indices: SparseTensor):
        ...
        out_pc = self.emb_cp(_out, components_indices)
        outbnfc1_pc = self.bnfc1(self.fc1(out_pc))
        final = self.fc2(self.relu(outbnfc1_pc))
        return final
...
class HRNetStyleCls3S2BD128(HRNetStyleClsBase):
    BLOCK = BasicBlock
    FEAT_FACTOR = 1
    NUM_STAGES = 3
    NUM_BLOCKS = 2
...
class HRNetStyleCls3S2BD256(HRNetStyleClsBase):
    BLOCK = BasicBlock
    FEAT_FACTOR = 1
    NUM_STAGES = 3
    NUM_BLOCKS = 2
...
class HRNetStyleCls1S2BD128(HRNetStyleClsBase):
    BLOCK = BasicBlock
    FEAT_FACTOR = 1
    NUM_STAGES = 1
    NUM_BLOCKS = 3
...
class HRNetStyleCls3S3BND256(HRNetStyleClsBase):
    BLOCK = Bottleneck
    FEAT_FACTOR = 1
    NUM_STAGES = 3
    NUM_BLOCKS = 2
...
class HRNetStyleCls3S3BNDF4256(HRNetStyleClsBase):
    BLOCK = Bottleneck
    FEAT_FACTOR = 4
    NUM_STAGES = 3
    NUM_BLOCKS = 3
...
class HRNetStyleCls3S2BND128(HRNetStyleClsBase):
    BLOCK = Bottleneck
    FEAT_FACTOR = 1
    NUM_STAGES = 3
    NUM_BLOCKS = 2
