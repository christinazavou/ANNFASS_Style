...
class StyleClsFromPsRes16UNetBase(ResNetBase):
  BLOCK = None
  ...
  def __init__(self, in_channels, out_channels, config, D=3, **kwargs):
    super(StyleClsFromPsRes16UNetBase, self).__init__(in_channels, out_channels, config, D)
  ...
  def network_initialization(self, in_channels, out_channels, config, D):
    ...
    self.emb_cp = MinkowskiPoolFeaturesPerComponent(self.PLANES[7])
  ...
  def load_from_ps(self, ps_checkpoint):
    if os.path.exists(ps_checkpoint):
      ...
      for name, param in self.named_parameters():
        if any(name.startswith(final_layer) for final_layer in ['block8', 'final']):
  ...
  def forward(self, x: SparseTensor, components_indices: SparseTensor):
    ...
    return self.final(out_pc)


class StyleClsFromPsRes16UNet34(StyleClsFromPsRes16UNetBase):
  BLOCK = BasicBlock
  LAYERS = (2, 3, 4, 6, 2, 2, 2, 2)


class StyleClsFromPsRes16UNet34A(StyleClsFromPsRes16UNet34):
