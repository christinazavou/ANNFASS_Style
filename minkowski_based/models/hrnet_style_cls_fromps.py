...
class HRNetStyleClsFromPsBase(Model):
    BLOCK = None
    NUM_STAGES = 1
    NUM_BLOCKS = 3
    INIT_DIM = 32
    ...
    def __init__(self, in_channels, out_channels, config, D=3, **kwargs):
        ...
        super(HRNetStyleClsFromPsBase, self).__init__(in_channels, out_channels, config, D, **kwargs)
    ...
    def final_classification_layers(self, D, backbone_out_feat, bn_momentum, out_channels):
        self.fc1 = conv(in_planes=backbone_out_feat, out_planes=self.CLASS_LAYER_DIM, kernel_size=1, stride=1, bias=True, D=D)
        self.bnfc1 = get_norm(norm_type=self.NORM_TYPE, n_channels=self.CLASS_LAYER_DIM, D=D, bn_momentum=bn_momentum)
        self.fc2 = conv(in_planes=self.CLASS_LAYER_DIM, out_planes=self.EMB_DIM, kernel_size=1, stride=1, bias=True, D=D)
        self.bnfc2 = get_norm(norm_type=self.NORM_TYPE, n_channels=self.EMB_DIM, D=D, bn_momentum=bn_momentum)
        self.emb_cp = MinkowskiPoolFeaturesPerComponent(self.EMB_DIM)
        self.fc3 = conv(in_planes=self.EMB_DIM, out_planes=out_channels, kernel_size=1, stride=1, bias=True, D=D)
        self.relu = MinkowskiReLU(inplace=True)

    def load_from_ps(self, ps_checkpoint):
        if os.path.exists(ps_checkpoint):
            logging.info('===> Loading ps weights in style net: ' + ps_checkpoint)
            state = torch.load(ps_checkpoint)['state_dict']
            for name, param in self.named_parameters():
                if any(name.startswith(final_layer) for final_layer in ['fc1', 'bnfc1', 'fc2', 'bnfc2', 'fc3']):
                    print(f"skip loading layer {name}")
                    continue
                if name in state:
    ...
    def forward(self, x: SparseTensor, components_indices: SparseTensor):
        ...
        out = self.relu(self.bnfc1(self.fc1(_out)))
        out = self.relu(self.bnfc2(self.fc2(out)))
        out_pc = self.emb_cp(out, components_indices)
        final = self.fc3(out_pc)
        return final
...
class HRNetStyleClsFromPs3S2BD256(HRNetStyleClsFromPsBase):
    BLOCK = BasicBlock
    FEAT_FACTOR = 1
    NUM_STAGES = 3
    NUM_BLOCKS = 2
