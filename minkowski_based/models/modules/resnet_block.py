...
class BottleneckBase(nn.Module):
  ...
  def __init__(self,
    ...
    self.conv1 = conv(inplanes, planes // self.bottleneck, kernel_size=1, D=D)
    self.norm1 = get_norm(self.NORM_TYPE, planes // self.bottleneck, D, bn_momentum=bn_momentum)

    self.conv2 = conv(
        planes // self.bottleneck, planes // self.bottleneck, kernel_size=3, stride=stride, dilation=dilation, conv_type=conv_type, D=D)
    self.norm2 = get_norm(self.NORM_TYPE, planes // self.bottleneck, D, bn_momentum=bn_momentum)

    self.conv3 = conv(planes // self.bottleneck, planes * self.expansion, kernel_size=1, D=D)
...
class Bottleneck2(BottleneckBase):
  NORM_TYPE = NormType.BATCH_NORM
  bottleneck = 2
