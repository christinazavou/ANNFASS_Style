...
# Modified by Marios Loizou for MinkowskiNet
import os
import json
import numpy as np
from tqdm import tqdm
from lib.pc_utils import save_point_cloud_with_only_normals


current_path = os.path.dirname(os.path.realpath(__file__))
root_folder = os.path.join(os.sep, "media", "melinos", "BigData", "ShapeNetRAW", "partnet_dataset")
...
def create_mink_net_dataset():
  # Base directories
  data_path = os.path.join(root_folder, 'data_v0')
  split_path = os.path.join(root_folder, 'partnet_dataset_master/stats/train_val_test_split')
  output_path = os.path.join(root_folder, 'minkowski_net')
  print('Convert the raw data to ply files ...')
  os.makedirs(output_path, exist_ok=True)

  for category in finegrained_category:
    # Convert data in .ply format for each finegrained category
    level_i = finest_level_dict[category]
    print("Convert data for {}, level {}".format(category, level_i))
    for phase in ['train', 'val', 'test']:
      print("{} split" .format(phase))
      split_json = os.path.join(split_path, "%s.%s.json" % (category, phase))
      split_output_path = os.path.join(output_path, category, phase)
      os.makedirs(split_output_path, exist_ok=True)
      with open(split_json, "r") as fid:
        name_json = json.load(fid)
      filenames = []
      for model in tqdm(name_json):
        filename = convert_single_data(data_path, split_output_path, model['anno_id'])
        filenames.append(filename)
      with open(os.path.join(output_path, category, phase + ".txt"), 'w') as fout:
        for fn in filenames:
          fout.write(os.path.join(phase, fn) + '\n')
...
def convert_ply():
  ...
  for anno_id in tqdm(os.listdir(data_path)):
...
def convert_single_data(data_path, output_path, anno_id):
  ...
  level_i = level_list_dict[category][-1] # !!! only consider the fine grained level
  ...
  processed = np.hstack((points, normals, labels[:, np.newaxis]))
  save_point_cloud_with_only_normals(processed, os.path.join(output_path, filename), with_label=True, verbose=False)
