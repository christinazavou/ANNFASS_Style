...
def save_point_cloud_with_only_normals(points_3d, filename, binary=True, with_label=False, verbose=True):
  """Save an normals point cloud as a PLY file.

  Args:
    points_3d: Nx6 matrix where points_3d[:, :3] are the XYZ coordinates and points_3d[:, 4:6] are the nx,ny,nz normals.
  """
  assert points_3d.ndim == 2
  if with_label:
    assert points_3d.shape[1] == 7
    python_types = (float, float, float, float, float, float, int)
    npy_types = [('x', 'f4'), ('y', 'f4'), ('z', 'f4'), ('nx', 'f4'), ('ny', 'f4'), ('nz', 'f4'), ('label', 'u1')]
  else:
    assert points_3d.shape[1] == 6
    python_types = (float, float, float, float, float, float)
    npy_types = [('x', 'f4'), ('y', 'f4'), ('z', 'f4'), ('nx', 'f4'), ('ny', 'f4'), ('nz', 'f4')]
  if binary is True:
    # Format into NumPy structured array
    vertices = []
    for row_idx in range(points_3d.shape[0]):
    ...
    with open(filename, 'w') as f:
      ...
      for row_idx in range(points_3d.shape[0]):
        if with_label:
          X, Y, Z, NX, NY, NZ, L = points_3d[row_idx]
          f.write('%f %f %f %f %f %f %d\n' % (X, Y, Z, NX, NY, NZ, L))
        else:
          X, Y, Z, NX, NY, NZ, = points_3d[row_idx]
          f.write('%f %f %f %f %f %f\n' % (X, Y, Z, NX, NY, NZ))
  if verbose is True:
    print('Saved point cloud to: %s' % filename)
