...
class VoxelizationDatasetBase(DictDataset, ABC):
  ...
  def __init__(self,
    ...
    if self.prefetch_data:
      self.prefetched_coords, self.prefetched_feats, self.prefetched_labels, self.prefetched_center = [], [], [], []
      for data_ind in tqdm(range(len(self.data_paths))):
        coords, feats, labels, center = self.load_ply(data_ind, self.input_feat)
        if self.normalize:
        ...
        self.prefetched_center.append(center)
      ...
      print("size in bytes of prefetched labels: {}".format(sys.getsizeof(self.prefetched_labels)))
    if self.rot_aug:
...
class VoxelizationDataset(VoxelizationDatasetBase):
  ...
  def __getitem__(self, index):
    ...
    if self.config.prefetch_data:
      ...
      center = np.copy(self.prefetched_center[index])
    else:
      coords, feats, labels, center = self.load_ply(index, self.input_feat)
      if self.normalize:
        coords = t.normalize_coords(coords, self.normalize_method)
