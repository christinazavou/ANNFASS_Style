...
class TranslationalJittering:
  ...
  def __call__(self, coords, feats, labels):
    N, C = coords.shape
    jittered_coords = np.clip(self.sigma * np.random.randn(N, C), -1 * self.clip, self.clip)
    jittered_coords += coords
...
def get_component_indices_matrix(component_ids, component_names):
  component_idx_list = sorted(list(set(component_ids) - {-1}))
  num_components = len(component_idx_list)
  num_points = len(component_ids)
  component_indices = np.zeros((num_components, num_points))
  for point_idx, c_idx in enumerate(component_ids):
    if c_idx != -1:
      c_idx = component_idx_list.index(c_idx)
      component_indices[c_idx, point_idx] = 1
  assert not any(np.sum(component_indices, 1) == 0)
  return component_indices, [component_names[i-1] for i in component_idx_list]


def get_average_per_component(component_indices_mat, point_vectors):
  outfeat = np.matmul(np.expand_dims(component_indices_mat, -2), np.expand_dims(point_vectors, -3))
  outfeat = np.squeeze(outfeat, 1) / np.expand_dims(np.sum(component_indices_mat, 1), -1)
  return outfeat
...
class cfl_collate_fn_factory:
  ...
  def __call__(self, list_data):
    coords, feats, labels, c_indices, c_names = list(zip(*list_data))
    coords_batch, feats_batch, labels_batch, c_indices_batch, c_names_batch = [], [], [], [], []
    ...
    for batch_id, _ in enumerate(coords):
      ...
      if self.limit_numpoints and batch_num_points > self.limit_numpoints:
        ...
        break
      new_c_indices = []
      for old_c_idx in c_indices[batch_id]:
        if old_c_idx != -1:
          new_c_indices.append(old_c_idx + num_components)
        else:
          new_c_indices.append(-1)
      num_components += len(c_names[batch_id])
      coords_batch.append(torch.from_numpy(coords[batch_id]).int())
      feats_batch.append(torch.from_numpy(feats[batch_id]))
      labels_batch.append(torch.from_numpy(labels[batch_id]).int())
      c_indices_batch.append(new_c_indices)
      c_names_batch.append(c_names[batch_id])

      batch_id += 1

    # Concatenate all lists
    coords_batch, feats_batch, labels_batch = ME.utils.sparse_collate(coords_batch, feats_batch, labels_batch)
    c_indices_batch = np.concatenate(c_indices_batch)
    c_names_batch = np.concatenate(c_names_batch)
    assert np.max(c_indices_batch) < len(c_names_batch)
    return coords_batch, feats_batch.float(), labels_batch, c_indices_batch, c_names_batch
...
class cflt_collate_fn_factory:
  ...
  def __call__(self, list_data):
    coords, feats, labels, c_indices, c_names, transformations = list(zip(*list_data))
    cfl_collate_fn = cfl_collate_fn_factory(limit_numpoints=self.limit_numpoints)
    coords_batch, feats_batch, labels_batch, c_indices_batch, c_names_batch = cfl_collate_fn(list(zip(coords, feats, labels, c_indices, c_names)))
    ...
    return coords_batch, feats_batch, labels_batch, c_indices_batch, c_names_batch, transformations_batch
