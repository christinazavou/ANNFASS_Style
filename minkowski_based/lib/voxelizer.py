...
class Voxelizer:
  ...
  def voxelize_both_together(self, coords, feats, labels, component_ids, center=None):
    # get the coordinate, features, style label and component_id of each sparse voxel
    # note: here we have all the points, and all of them have a style label (can be "unknown")
    # ignore_label (i.e. -1) will be used for the cases where a voxel has various labels
    # as well as for component id that i dont care
    # note: because of the point augmentation, if i called this function twice i could get different points and thus
    # different sparse voxels
    assert coords.shape[1] == 3 and coords.shape[0] == feats.shape[0] and coords.shape[0]
    if self.clip_bound is not None:
      trans_aug_ratio = np.zeros(3)
      if self.use_augmentation and self.translation_augmentation_ratio_bound is not None:
      ...
      if clip_inds is not None:
        ...
        if component_ids is not None:
          component_ids = component_ids[clip_inds]
    ...
    _, _, labels = ME.utils.sparse_quantize(
        coords_aug, feats, labels=labels, ignore_label=self.ignore_label)

    coords_aug, feats, component_ids = ME.utils.sparse_quantize(
        coords_aug, feats, labels=component_ids, ignore_label=self.ignore_label)

    return coords_aug, feats, labels, component_ids, rigid_transformation.flatten()
