...
def train_test(train_data_loader, test_data_loader, pkl_filename):
    ...
    relevance_class_idx = list(clf.classes_).index(1)
    test_target_relevance = test_pred[::2, relevance_class_idx]
    test_target_irrelevance = test_pred[1::2, relevance_class_idx]
    found = (test_target_relevance > test_target_irrelevance)
    acc = np.sum(found) / len(found)
    return acc
