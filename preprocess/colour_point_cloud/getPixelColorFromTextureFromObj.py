...
"""
Pratheba's code with minor changes to run on cypriot buildings.
"""
...
#                        ni = (float)(line.split()[1])
                    if line.startswith("map_Kd "):
                        if os.path.exists(line.split("map_Kd ")[1]):
                            c.tex = os.path.basename(line.split("map_Kd ")[1][0:-4])
            if len(matname):
                self.materialToColor[matname] = copy.deepcopy(c)
...
#        print(P_V)
    ...
    def createMipMap(self, img, imgfilename, width, height):
        ...
        dbg_f = os.path.join(f"/home/christina/Documents/annfass_playground/debug_colour/MipMaps/{imgfilename}_{width}_{height}.jpg")
        Image.fromarray(np.array(img_downsize)).save(dbg_f)
        return self.imgToPixel[imgfilename]
    ...
    def getAllMipMaps(self, imgfilepath, imgfilename):
        ...
        #else:
        if ext.lower() == '.gif' or ext.lower() == '.png':
            img = img.convert('RGB')
...
#        print(material)
    ...
    def writePLY(self, destFolder):
        ...
        for line in fread:
            ...
            f.write(" ".join(line.split(" ")[:-3]) + f" {int(self.P_color[index][0])} {int(self.P_color[index][1])} {int(self.P_color[index][2])} {int(self.P_color[index][3])}\n")
            index += 1
            #print(index)
        f.close()
