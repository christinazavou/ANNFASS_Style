...
if __name__ == '__main__':
    ...
    parser.add_argument("--class", type=str, dest='_class')
    parser.add_argument("--random_rot", type=int, default=0, help="Default would be 0 for no rotations or 10 otherwise")
    ...
    for model_dir in os.listdir(ARGS.inp_dir):
        ...
        if ARGS._class in ['chair', 'furniture', 'car', 'building']:
            rot = mathutils.Matrix.Rotation(math.radians(90), 3, (1, 0, 0))
            vertices = np.matmul(vertices, rot)
            rot = mathutils.Matrix.Rotation(math.radians(-90), 3, (0, 1, 0))
            vertices = np.matmul(vertices, rot)
