...
parser.add_argument("--full_path", type=str, default="False")
...
with open(txt_file, "w") as fout:
    for building_file in building_files:
        if any(building in building_file for building in buildings):
            found_buildings.append(building_file)
            if eval(FLAGS.full_path):
                fout.write(f"{FLAGS.data_dir}/{building_file}.ply\n")
            else:
                fout.write(f"{building_file}.ply\n")
