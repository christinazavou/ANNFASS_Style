...
class IM_AE(object):
    def __init__(self, config):
        ...
        if os.path.exists(self.data_style):

            # load data
            fin = open(self.data_style)
            ...
            if config.train:
                for i in range(self.styleset_len):
                    ...
                    if self.output_size == 128:
                        tmp_raw = get_vox_from_binvox_1over2(
                            os.path.join(self.data_dir, self.styleset_names[i] + "/model_depth_fusion.binvox")).astype(
                            np.uint8)
                    elif self.output_size == 256:
                        tmp_raw = get_vox_from_binvox(
                            os.path.join(self.data_dir, self.styleset_names[i] + "/model_depth_fusion.binvox")).astype(
                            np.uint8)
            ...
            print("ERROR: cannot load styleset txt: " +self.data_style )
            exit(-1)
        ...
        if os.path.exists(self.data_content):

            # load data
            fin = open(self.data_content)
            ...
            if config.train:
                for i in range(self.dataset_len):
                    ...
                    if self.output_size == 128:
                        tmp_raw = get_vox_from_binvox_1over2(
                            os.path.join(self.data_dir, self.dataset_names[i] + "/model_depth_fusion.binvox")).astype(
                            np.uint8)
                    elif self.output_size == 256:
                        tmp_raw = get_vox_from_binvox(
                            os.path.join(self.data_dir, self.dataset_names[i] + "/model_depth_fusion.binvox")).astype(
            ...
            print("ERROR: cannot load dataset txt: " +  self.data_content )
            exit(-1)
    ...
    def crop_voxel(self, vox, xmin, xmax, ymin, ymax, zmin, zmax):
        ...
        zspan = zmax - zmin
        tmp = np.zeros(
            [xspan * self.upsample_rate + self.mask_margin * 2, yspan * self.upsample_rate + self.mask_margin * 2,
             zspan * self.upsample_rate + self.mask_margin * 2], np.uint8)
        if self.asymmetry:
            tmp[self.mask_margin:-self.mask_margin, self.mask_margin:-self.mask_margin,
            ...
            tmp[self.mask_margin:-self.mask_margin, self.mask_margin:-self.mask_margin, :-self.mask_margin] = vox[
    ...
    def train(self, config):
        ...
        for epoch in range(0, training_epoch):
            ...
            for idx in range(self.dataset_len):
                ...
                voxel_fake = voxel_fake.detach()  #notice that it is detached when passed to dicriminator in Dstep ;)
                ...
                for dstep in range(d_step):
                    ...
                    Dmask_style = torch.from_numpy(self.Dmask_style[qxp]).to(self.device).unsqueeze(0).unsqueeze(
                        0).float()

                    D_out = self.discriminator(voxel_style, is_training=True)
                    loss_d_real = (torch.sum(
                        (D_out[:, z_vector_style_idx:z_vector_style_idx + 1] - 1) ** 2 * Dmask_style) + torch.sum(
                        (D_out[:, -1:] - 1) ** 2 * Dmask_style)) / torch.sum(Dmask_style)
                    loss_d_real.backward()

                    D_out = self.discriminator(voxel_fake, is_training=True)
                    loss_d_fake = (torch.sum(
                        (D_out[:, z_vector_style_idx:z_vector_style_idx + 1]) ** 2 * Dmask_fake) + torch.sum(
                        (D_out[:, -1:]) ** 2 * Dmask_fake)) / torch.sum(Dmask_fake)
                ...
                for rstep in range(r_step):
                    ...
                    voxel_style = torch.from_numpy(self.voxel_style[qxp]).to(self.device).unsqueeze(0).unsqueeze(0)
                    mask_style = torch.from_numpy(self.mask_style[qxp]).to(self.device).unsqueeze(0).unsqueeze(
                        0).float()
                    input_style = torch.from_numpy(self.input_style[qxp]).to(self.device).unsqueeze(0).unsqueeze(
                        0).float()
            ...
            epoch, training_epoch, time.time() - start_time, loss_d_real.item(), loss_d_fake.item(), loss_r.item(),
            loss_g.item()))
    ...
    def test(self, config):
        ...
        for i in range(self.styleset_len):
            if self.output_size == 128:
                tmpvox = get_vox_from_binvox_1over2(
                    os.path.join(self.data_dir, self.styleset_names[i] + "/model_depth_fusion.binvox")).astype(np.uint8)
            elif self.output_size == 256:
                tmpvox = get_vox_from_binvox(
                    os.path.join(self.data_dir, self.styleset_names[i] + "/model_depth_fusion.binvox")).astype(np.uint8)
        ...
        for i in range(self.dataset_len):
            if self.output_size == 128:
                tmp_raw = get_vox_from_binvox_1over2(
                    os.path.join(self.data_dir, self.dataset_names[i] + "/model_depth_fusion.binvox")).astype(np.uint8)
            elif self.output_size == 256:
                tmp_raw = get_vox_from_binvox(
                    os.path.join(self.data_dir, self.dataset_names[i] + "/model_depth_fusion.binvox")).astype(np.uint8)
            xmin, xmax, ymin, ymax, zmin, zmax = self.get_voxel_bbox(tmp_raw)
            ...
            for x in range(row_num):
                for y in range(col_num):
                    ...
                    tmp_voxel_fake = voxel_fake.detach().cpu().numpy()[0, 0]
                    tmpvox = self.recover_voxel(tmp_voxel_fake, xmin, xmax, ymin, ymax, zmin, zmax)
                    self.imgout_0[img_y * self.real_size:(img_y + 1) * self.real_size, img_x * self.real_size:(
                                                                                                                          img_x + 1) * self.real_size] = self.voxel_renderer.render_img_with_camera_pose_gpu(
                        tmpvox, self.sampling_threshold)
    ...
    def prepare_voxel_style(self, config):
        ...
        for style_id in range(self.styleset_len):
            ...
            if self.output_size == 128:
                tmp_raw = get_vox_from_binvox_1over2(
                    os.path.join(self.data_dir, self.styleset_names[style_id] + "/model_depth_fusion.binvox")).astype(
                    np.uint8)
            elif self.output_size == 256:
                tmp_raw = get_vox_from_binvox(
                    os.path.join(self.data_dir, self.styleset_names[style_id] + "/model_depth_fusion.binvox")).astype(
    ...
    def prepare_voxel_for_eval(self, config):
        ...
        for content_id in range(self.dataset_len):
            ...
            if self.output_size == 128:
                tmp_raw = get_vox_from_binvox_1over2(
                    os.path.join(self.data_dir, self.dataset_names[content_id] + "/model_depth_fusion.binvox")).astype(
                    np.uint8)
            elif self.output_size == 256:
                tmp_raw = get_vox_from_binvox(
                    os.path.join(self.data_dir, self.dataset_names[content_id] + "/model_depth_fusion.binvox")).astype(
    ...
    def render_fake_for_eval(self, config):
        ...
        for content_id in range(self.dataset_len):
            ...
            tmpvoxlarger = np.zeros(
                [self.real_size + render_boundary_padding_size * 2, self.real_size + render_boundary_padding_size * 2,
            ...
            for style_id in range(min(self.styleset_len, max_num_of_styles)):
                ...
                if self.asymmetry:
                    ...
                    zmin2 + render_boundary_padding_size:zmax2 + render_boundary_padding_size] = tmp_voxel_fake[::-1,
                                                                                                 ::-1, :]
                else:
                    tmpvoxlarger[xmin2 + render_boundary_padding_size:xmax2 + render_boundary_padding_size,
                    ymin2 + render_boundary_padding_size:ymax2 + render_boundary_padding_size,
                    ...
                    tmpvoxlarger[xmin2 + render_boundary_padding_size:xmax2 + render_boundary_padding_size,
                    ymin2 + render_boundary_padding_size:ymax2 + render_boundary_padding_size,
                    zmin2 - 1 + render_boundary_padding_size:zmin2 * 2 - zmax2 - 1 + render_boundary_padding_size:-1] = tmp_voxel_fake[
                                                                                                                        ::-1,
                                                                                                                        ::-1,
                                                                                                                        self.mask_margin:]
                ...
                for sample_id in range(sample_num_views):
                    ...
                    tmpvoxlarger_tensor = torch.from_numpy(tmpvoxlarger).to(self.device).unsqueeze(0).unsqueeze(
                        0).float()
                    imgout = self.voxel_renderer.render_img_with_camera_pose_gpu(tmpvoxlarger_tensor,
                                                                                 self.sampling_threshold, cam_alpha,
                                                                                 cam_beta, get_depth=False,
                                                                                 processed=True)
                    if self.output_size == 128:
                        imgout = cv2.resize(imgout, (self.real_size * 2, self.real_size * 2),
                                            interpolation=cv2.INTER_NEAREST)
                        imgout = imgout[half_real_size:-half_real_size, half_real_size:-half_real_size]
                    cv2.imwrite(
                        result_dir + "/" + str(content_id) + "_" + str(style_id) + "_" + str(sample_id) + ".png",
                        imgout)
    ...
    def render_real_for_eval(self, config):
        ...
        for content_id in range(self.dataset_len):
            ...
            for sample_id in range(sample_num_views):
                cam_alpha = np.random.random() * np.pi * 2
                cam_beta = np.random.random() * np.pi / 2 - np.pi / 4
                imgout = self.voxel_renderer.render_img_with_camera_pose_gpu(tmp_raw, self.sampling_threshold,
                                                                             cam_alpha, cam_beta, get_depth=False,
                                                                             processed=False)
                if self.output_size == 128:
                    imgout = cv2.resize(imgout, (self.real_size * 2, self.real_size * 2),
                                        interpolation=cv2.INTER_NEAREST)
    ...
    def launch_ui(self, config):
        ...
        while True:
            ...
            if mouse_xyd[0] != mouse_xyd_backup[0] or mouse_xyd[1] != mouse_xyd_backup[1] or mouse_xyd[2] != \
                    mouse_xyd_backup[2]:

                if mouse_xyd[0] < UI_imgheight and mouse_xyd[1] < UI_imgheight:  # inside output rergion
                    if mouse_xyd_backup[0] < UI_imgheight and mouse_xyd_backup[1] < UI_imgheight and mouse_xyd[
                        2] == 1 and mouse_xyd_backup[2] == 1:
                ...
                elif mouse_xyd[0] >= scrollbar_offset_x and mouse_xyd[0] < scrollbar_offset_x2 and mouse_xyd[
                    1] >= scrollbar_offset_y and mouse_xyd[1] < scrollbar_offset_y2 + 20:  # inside scrollbar
                    if mouse_xyd[2] == 1:
                ...
                elif mouse_xyd[0] >= content_offset_x and mouse_xyd[0] < content_offset_x + content_textlen and \
                        mouse_xyd[1] >= scrollbar_offset_y and mouse_xyd[
                    1] < scrollbar_offset_y2:  # inside content shape browser
            ...
            if content_id_changed_flag:
                ...
                if self.output_size == 128:
                    tmp_raw = get_vox_from_binvox_1over2(os.path.join(self.data_dir, self.dataset_names[
                        content_id] + "/model_depth_fusion.binvox")).astype(np.uint8)
                elif self.output_size == 256:
                    tmp_raw = get_vox_from_binvox(os.path.join(self.data_dir, self.dataset_names[
                        content_id] + "/model_depth_fusion.binvox")).astype(np.uint8)
                xmin, xmax, ymin, ymax, zmin, zmax = self.get_voxel_bbox(tmp_raw)
                tmp = self.crop_voxel(tmp_raw, xmin, xmax, ymin, ymax, zmin, zmax)
                ...
                contentvox = np.zeros([self.real_size + render_boundary_padding_size * 2,
                                       self.real_size + render_boundary_padding_size * 2,
                                       self.real_size + render_boundary_padding_size * 2], np.float32)
                ...
                if self.asymmetry:
                    contentvox[xmin2 + render_boundary_padding_size:xmax2 + render_boundary_padding_size,
                    ymin2 + render_boundary_padding_size:ymax2 + render_boundary_padding_size,
                    zmin2 + render_boundary_padding_size:zmax2 + render_boundary_padding_size] = tmp_voxel_fake[::-1,
                                                                                                 ::-1, :]
                else:
            ...
            img = self.voxel_renderer.render_img_with_camera_pose_gpu(contentvox_tensor, self.sampling_threshold,
                                                                      cam_alpha, cam_beta, get_depth=False,
                                                                      processed=True)
            img = cv2.resize(img, (content_img_size, content_img_size))
            UI_image[content_img_offset_y:content_img_offset_y + content_img_size,
            content_img_offset_x:content_img_offset_x + content_img_size] = np.reshape(img, [content_img_size,
                                                                                             content_img_size, 1])
            ...
            if z_vector_changed_flag:
                ...
                outputvox = np.zeros([self.real_size + render_boundary_padding_size * 2,
                                      self.real_size + render_boundary_padding_size * 2,
                                      self.real_size + render_boundary_padding_size * 2], np.float32)
                ...
                if self.asymmetry:
                    outputvox[xmin2 + render_boundary_padding_size:xmax2 + render_boundary_padding_size,
                    ymin2 + render_boundary_padding_size:ymax2 + render_boundary_padding_size,
                    zmin2 + render_boundary_padding_size:zmax2 + render_boundary_padding_size] = tmp_voxel_fake[::-1,
                                                                                                 ::-1, :]
                else:
            ...
            img = self.voxel_renderer.render_img_with_camera_pose_gpu(outputvox_tensor, self.sampling_threshold,
                                                                      cam_alpha, cam_beta, get_depth=False,
                                                                      processed=True)
