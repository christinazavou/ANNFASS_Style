...
from runners.common_dp2 import IM_AE_ANY_COMMON
...
class IM_AE(IM_AE_ANY_COMMON):
    ...
    def train(self, config):
        ...
        triplet_criterion = torch.nn.MarginRankingLoss(margin=config.margin)
        ...
        while epoch < training_epoch:
            ...
            for idx in range(epoch_size):
                ...
                while style_idx == other_style_idx:
                    other_style_idx = np.random.randint(self.styleset_len)
                ...
                z_tensor_g = self.discriminator.pool(D_out_real)[:, 0:-1, :, :, :]  # fixme: need any detach or smth?
                ...
                for dstep in range(config.d_steps):  # notice that generator is not reused in each dstep as its not updated
                    ...
                    D_out_real = self.discriminator(voxel_style, is_training=True)
                    # loss of "real specific style"
                    loss_d_real = torch.sum((D_out_real[:, -1:] - 1) ** 2 * Dmask_style) / torch.sum(Dmask_style)
                    loss_d_real.backward(retain_graph=True)

                    real_patches1 = get_random_non_cube_patches(D_out_real[0, :-1], patch_factor=2, patch_num=20, stride_factor=2)
                    real_patches1 = self.discriminator.pool(real_patches1).squeeze(4).squeeze(3).squeeze(2)

                    real_patches2 = get_random_non_cube_patches(D_out_real[0, :-1], patch_factor=2, patch_num=20, stride_factor=2)
                    real_patches2 = self.discriminator.pool(real_patches2).squeeze(4).squeeze(3).squeeze(2)

                    D_out_fake = self.discriminator(voxel_fake, is_training=True)
                    # loss of "fake specific style"
                    loss_d_fake = torch.sum((D_out_fake[:, -1:]) ** 2 * Dmask_fake) / torch.sum(Dmask_fake)
                    loss_d_fake.backward(retain_graph=True)

                    fake_patches = get_random_non_cube_patches(D_out_fake[0, :-1], patch_factor=2, patch_num=20, stride_factor=2)
                    fake_patches = self.discriminator.pool(fake_patches).squeeze(4).squeeze(3).squeeze(2)

                    D_out_other = self.discriminator(voxel_other_style, is_training=True)

                    real_other_patches = get_random_non_cube_patches(D_out_other[0, :-1], patch_factor=2, patch_num=20, stride_factor=2)
                    real_other_patches = self.discriminator.pool(real_other_patches).squeeze(4).squeeze(3).squeeze(2)

                    # patch contrastive loss
                    dist_anchor_pos = F.pairwise_distance(real_patches1, real_patches2, 2)
                    dist_anchor_neg = F.pairwise_distance(real_patches1, real_other_patches, 2)

                    target = torch.FloatTensor(dist_anchor_pos.size()).fill_(-1)  # -1 means dist_anchor_neg should be bigger than dist_anchor_pos
                    target = Variable(target.to(self.device))
                    patch_loss_d_ground = triplet_criterion(dist_anchor_pos, dist_anchor_neg, target)
                    patch_loss_d_ground.backward(retain_graph=True)

                    dist_anchor_pos = F.pairwise_distance(real_patches1, real_patches2, 2)
                    dist_anchor_neg = F.pairwise_distance(real_patches1, fake_patches, 2)

                    target = torch.FloatTensor(dist_anchor_pos.size()).fill_(-1)  # -1 means dist_anchor_neg should be bigger than dist_anchor_pos
                    target = Variable(target.to(self.device))
                    patch_loss_d_fake = triplet_criterion(dist_anchor_pos, dist_anchor_neg, target)
                    patch_loss_d_fake.backward(retain_graph=True)
                ...
                for rstep in range(r_steps):
                    ...
                    self.discriminator.zero_grad()  # todo: use both optimizers in reconstruction?

                    D_out_real = self.discriminator(voxel_style_2, is_training=True)
                    z_tensor2_g = self.discriminator.pool(D_out_real)[:, 0:-1, :, :, :]
                ...
                # G step
                D_out_real = self.discriminator(voxel_style, is_training=True)
                z_tensor_g = self.discriminator.pool(D_out_real)[:, 0:-1, :, :, :]
                z_tensor_g.detach()  # fixme: need any detach or smth?
                ...
                for gstep in range(self.g_steps):
                    ...
                    # todo: need any use of self.param_alpha?!
                    # global plausibility loss
                    loss_g = torch.sum((D_out_fake[:, -1:] - 1) ** 2 * Dmask_fake) / torch.sum(Dmask_fake)
                    loss_g.backward(retain_graph=True)

                    # patch contrastive loss
                    real_patches = get_random_non_cube_patches(D_out_real[0, :-1], patch_factor=2, patch_num=20, stride_factor=2)
                    real_patches = self.discriminator.pool(real_patches).squeeze(4).squeeze(3).squeeze(2)
                    fake_patches = get_random_non_cube_patches(D_out_fake[0, :-1], patch_factor=2, patch_num=20, stride_factor=2)
                    fake_patches = self.discriminator.pool(fake_patches).squeeze(4).squeeze(3).squeeze(2)

                    dist_real_fake = F.pairwise_distance(real_patches, fake_patches, 2)

                    patch_loss_g = torch.sum(dist_real_fake) * self.param_alpha  # fixme: correct ?
                    patch_loss_g.backward(retain_graph=True)
                ...
                if iter_counter % config.log_iter == 0:
                    print(
                        ...
                        "patch_loss_d_fake: %.6f, loss_r: %.6f, loss_g: %.6f, patch_loss_g: %.6f" % (
                            ...
                            patch_loss_d_ground.item(), patch_loss_d_fake.item(), loss_r.item(), loss_g.item(),
                            patch_loss_g.item()))
                    self.log.debug(
                        "Epoch: [%d/%d] time: %.0f, loss_d_real: %.6f, loss_d_fake: %.6f, patch_loss_d_ground: %.6f, "
                        "patch_loss_d_fake: %.6f, loss_r: %.6f, loss_g: %.6f, patch_loss_g: %.6f" % (
                    ...
                    train_writer.add_scalar('patch_loss_d_ground', patch_loss_d_ground.item(), iter_counter)
                    train_writer.add_scalar('patch_loss_d_fake', patch_loss_d_fake.item(), iter_counter)
                    train_writer.add_scalar('loss_r', loss_r.item(), iter_counter)
                    train_writer.add_scalar('loss_g', loss_g.item(), iter_counter)
                    train_writer.add_scalar('patch_loss_g', patch_loss_g.item(), iter_counter)
