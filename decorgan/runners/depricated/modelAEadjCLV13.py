...
class IM_AE(object):
    ...
    def visualise_init(self, sample_dir):
        ...
        for style_idx in range(0, 5):
            style_reference_file = self.style_set.get_reference_file(style_idx)
    ...
    def test(self, config):
        ...
        for i in range(self.dataset_len):
            ...
            content_data_dict = self.dset.__getitem__(i)
            mask_content = content_data_dict['mask']
            input_content = content_data_dict['input']
            xmin, xmax, ymin, ymax, zmin, zmax = content_data_dict['pos']
    ...
    def test_fig_3(self, config):
        ...
        for content_idx in range(self.dataset_len):
            img_coa, img_det, img_ful = self.coarse_detailed_full_plots(content_idx, content=True)
        ...
        for style_idx in range(self.styleset_len):
            if style_idx not in [6,17,21,22,25,27,3,15,20]:
                continue

            img_coa, img_det, img_ful = self.coarse_detailed_full_plots(style_idx, content=False)
            render_example([img_coa,
                            img_det,
                            img_ful],
            ...
            for content_idx in range(self.dataset_len):
                ...
                tmp_voxel_fake = voxel_fake.detach().cpu().numpy()[0, 0]
                img_coa, img_det, img_ful = self.generation_plot(tmp_voxel_fake, xmin, xmax, ymin, ymax, zmin, zmax)

                render_example([img_coa,
                                img_det,
                                img_ful],
                               save=os.path.join(config.sample_dir, f"content{content_idx}style{style_idx}.png"),
                               titles=['Coarse', 'Detailed Smooth', 'Detailed Full'])
