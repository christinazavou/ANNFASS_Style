...
class IM_AE(object):
    ...
    def __init__(self, config):
        ...
        if config.train:
            for i in range(iter_size):
                print("preprocessing shape as style - "+str(i+1)+"/"+str(self.dataset_len))
                self.log.debug("preprocessing shape as style - "+str(i+1)+"/"+str(self.dataset_len))
                data_dict = self.dset.__getitem__(i)
                xmin, xmax, ymin, ymax, zmin, zmax = data_dict['pos']
                voxel_style = data_dict['voxel_style']
        ...
        if config.train:
            for i in range(iter_size):
                print("preprocessing shape as content - "+str(i+1)+"/"+str(self.dataset_len))
                self.log.debug("preprocessing shape as content - "+str(i+1)+"/"+str(self.dataset_len))
                data_dict = self.dset.__getitem__(i)
                xmin, xmax, ymin, ymax, zmin, zmax = data_dict['pos']
    ...
    def visualise(self, sample_dir, epoch):
        ...
        for style_idx in range(5, 10):
            ...
            self.imgout_0[0:self.real_size, (style_idx-5+1)*self.real_size:(style_idx-5+2)*self.real_size] = detailed_full_style_img
        ...
        for content_idx in range(5):
            ...
            for style_idx in range(5, 10):
                ...
                self.imgout_0[(content_idx+1)*self.real_size: (content_idx+2)*self.real_size,
                              (style_idx-5+1)*self.real_size: (style_idx-5+2)*self.real_size] = detailed_img_smooth
