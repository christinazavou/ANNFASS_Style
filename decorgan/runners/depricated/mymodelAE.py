...
from utils.matplotlib_utils import render_result, render_example, render_views
import mcubes
...
class IM_AE(object):
    ...
    def __init__(self, config):
        ...
        self.optimizer_g = torch.optim.Adam(list(self.generator.parameters())+list(self.style_encoder.parameters()),
                                            lr=config.lr)
    ...
    def train(self, config):
        ...
        for epoch in range(0, training_epoch):
            ...
            epoch, training_epoch, time.time() - start_time, loss_d_real.item(), loss_d_fake.item(), loss_r.item(),
            loss_g.item()))
            train_writer.add_scalar('loss_d_real', loss_d_real.item(), epoch*self.dataset_len)
            train_writer.add_scalar('loss_d_fake', loss_d_fake.item(), epoch*self.dataset_len)
            train_writer.add_scalar('loss_r', loss_r.item(), epoch*self.dataset_len)
            train_writer.add_scalar('loss_g', loss_g.item(), epoch*self.dataset_len)
    ...
    def visualise_init(self, sample_dir):
        ...
        for content_idx in range(5):
            ...
            # content_reference_img = csvr(content_reference_file)
            # content_input_file = self.dset.get_input_file(content_idx)
            # content_input_img = img_of_input(content_input_file)
            ...
            render_example([coarse_img,
                           ...
                           title=title,
                           titles=['Coarse', 'Detailed Smooth', 'Detailed Full'])
            # render_example([coarse_mcubes_img,
            #                detailed_mcubes_smooth_img,
            #                detailed_mcubes_full_img],
            #                save=os.path.join(sample_dir, f"content{content_idx}_mcubes.png"),
            #                title=title,
            #                titles=['Coarse', 'Detailed Smooth', 'Detailed Full'])

        for style_idx in range(3, 8):
            style_reference_file = self.dset.get_reference_file(style_idx)
            # style_reference_img = csvr(style_reference_file)
            # style_input_file = self.dset.get_input_file(style_idx)
            # style_input_img = img_of_input(style_input_file)
            ...
            #                detailed_mcubes_smooth_img,
            #                detailed_mcubes_full_img],
            #                save=os.path.join(sample_dir, f"style{style_idx}_mcubes.png"),
            #                title=title,
            #                titles=['Coarse', 'Detailed Smooth', 'Detailed Full'])
        ...
        for content_idx in range(5):
            ...
            for style_idx in range(5, 10):
                ...
                render_example([coarse_img,
                               detailed_img_smooth,
                               detailed_img_full],
                               save=os.path.join(sample_dir,f"{epoch}_{content_idx}_{style_idx}.png"),
                               title=f'Generated',
                               titles=['Generated Coarse', 'Generated Smooth', 'Generated Full'])
                # render_example([mcubes_coarse_img,
                #                mcubes_det_smooth_img,
                #                mcubes_det_full_img],
                #                save=os.path.join(sample_dir,f"{epoch}_{content_idx}_{style_idx}_mcubes.png"),
                #                title=f'Generated')
    ...
    def test(self, config):
        ...
        plt_original.imshow(style_codes, cmap='hot', interpolation='nearest')
        plt_original.show()
    ...
    def test_fig_3(self, config):
        ...
        for content_idx in range(3):
            img_coa, img_det, img_ful = self.coarse_detailed_full_plots(content_idx)
            render_example([img_coa,
                            img_det,
                            img_ful],
                           save=os.path.join(config.sample_dir, f"content{content_idx}.png"),
                           titles=['Coarse', 'Detailed Smooth', 'Detailed Full'])

        for style_idx in range(3, 14):

            img_coa, img_det, img_ful = self.coarse_detailed_full_plots(style_idx)
            render_example([img_coa,
                            img_det,
                            img_ful],
                           save=os.path.join(config.sample_dir, f"style{style_idx}.png"),
                           titles=['Coarse', 'Detailed Smooth', 'Detailed Full'])
            ...
            for content_idx in range(3):
                ...
                tmp_voxel_fake = voxel_fake.detach().cpu().numpy()[0, 0]
                coarse_img, detailed_img_smooth, detailed_img_full = \
                    self.generation_plot(tmp_voxel_fake, xmin, xmax, ymin, ymax, zmin, zmax)

                voxel_fake_unmasked = self.generator.forward_without_mask(input_fake, z_tensor_g, is_training=False)

                tmp_voxel_fake_unmasked = voxel_fake_unmasked.detach().cpu().numpy()[0, 0]
                coarse_img_unmasked, detailed_img_smooth_unmasked, detailed_img_full_unmasked = \
                    self.generation_plot(tmp_voxel_fake_unmasked, xmin, xmax, ymin, ymax, zmin, zmax)

                render_example([coarse_img,
                                detailed_img_smooth,
                                detailed_img_full,
                                detailed_img_full_unmasked],
                               save=os.path.join(config.sample_dir, f"{content_idx}_{style_idx}.png"),
                               title=f'Generated',
                               titles=['Generated Coarse', 'Generated Smooth', 'Generated Full',
                                       'Generated Unmasked Full'])
