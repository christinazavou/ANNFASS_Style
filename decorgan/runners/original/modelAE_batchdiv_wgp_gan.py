...
class IM_AE(IM_AE_ORIG_COMMON):
    ...
    def __init__(self, config):
        ...
        # build model
        self.discriminator = discriminator_wgp(self.d_dim, self.styleset_len + 1, init_weights=config.init_weights)
    ...
    def discriminator_step(self, content_batch, style_batch, store_grads=False, LAMBDA=10):  # real and fake are detailed shapes
        len_batch = len(content_batch)
        self.discriminator.zero_grad()

        loss_d_real_value = 0.
        loss_d_fake_value = 0.
        for (coarse_content, detailed_content, mask_content, Dmask_content), \
            ...
            loss_d_real_style = - torch.sum((d_out_style) * Dmask_style) / torch.sum(Dmask_style)
            loss_d_real_global = - torch.sum((d_out_global) * Dmask_style) / torch.sum(Dmask_style)
            ...
            d_out_global = D_out[:, -1:]
            d_out_style = D_out[:, style_idx:style_idx + 1]
            loss_d_fake_style = torch.sum((d_out_style) * Dmask_fake) / torch.sum(Dmask_fake)
            loss_d_fake_global = torch.sum((d_out_global) * Dmask_fake) / torch.sum(Dmask_fake)
            ...
            epsilon = torch.rand(1).cuda().requires_grad_()
            ...
            # FIXME: is it ok that real and fake come from same shape (due to each shape having other size) ? :/
            G_interpolation = epsilon * detailed_content + (1 - epsilon) * voxel_fake
            D_interpolation = self.generator(G_interpolation)

            # Compute the gradients of D with respect to the noise generated input
            weight = torch.ones(D_interpolation.size()).cuda()
            ...
            loss_d = loss_d_real + loss_d_fake + grad_penalty
            loss_d.backward()
    ...
    def generator_step(self, content_batch, style_batch, store_grads=False):
        ...
        for (coarse_content, detailed_content, mask_content, Dmask_content), \
            ...
            loss_g_global = - torch.sum((d_out_style) * Dmask_fake) * self.param_alpha / torch.sum(Dmask_fake)
            loss_g_style = - torch.sum((d_out_global) * Dmask_fake) / torch.sum(Dmask_fake)
    ...
    def disc_gen_eval_step(self, content_batch, style_batch):  # real and fake are detailed shapes
        ...
        for (coarse_content, mask_content, Dmask_content), \
            ...
            loss_g = (torch.sum((D_out[:,
                                 style_idx:style_idx + 1] - 1) ** 2 * Dmask_fake) * self.param_alpha + torch.sum(
        ...
        while epoch < training_epoch:
            ...
            for idx in range(0, epoch_size-config.batch_size, config.batch_size):
                ...
                for b_idx in range(config.batch_size):
                    ...
                    detailed_content = content_data_dict['voxel_style']
                    ...
                    content_batch.append((input_content, detailed_content, mask_content, Dmask_content))
