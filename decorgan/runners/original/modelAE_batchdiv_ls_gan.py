...
class IM_AE(IM_AE_ORIG_COMMON):
    ...
    def discriminator_step(self, content_batch, style_batch, store_grads=False):  # real and fake are detailed shapes
        ...
        for (coarse_content, mask_content, Dmask_content), \
            ...
            loss_d_real_style = torch.sum((d_out_style - 1) ** 2 * Dmask_style) / torch.sum(Dmask_style)
            loss_d_real_global = torch.sum((d_out_global - 1) ** 2 * Dmask_style) / torch.sum(Dmask_style)
            loss_d_real = 0.5 * loss_d_real_style + 0.5 * loss_d_real_global
            ...
            d_out_global = D_out[:, -1:]
            d_out_style = D_out[:, style_idx:style_idx + 1]
            loss_d_fake_style = torch.sum((d_out_style) ** 2 * Dmask_fake) / torch.sum(Dmask_fake)
            loss_d_fake_global = torch.sum((d_out_global) ** 2 * Dmask_fake) / torch.sum(Dmask_fake)
            loss_d_fake = 0.5 * loss_d_fake_style + 0.5 * loss_d_fake_global
    ...
    def generator_step(self, content_batch, style_batch, store_grads=False):
        ...
        for (coarse_content, mask_content, Dmask_content), \
            ...
            loss_g_global = torch.sum((d_out_style - 1) ** 2 * Dmask_fake) * self.param_alpha / torch.sum(Dmask_fake)
            loss_g_style = torch.sum((d_out_global - 1) ** 2 * Dmask_fake) / torch.sum(Dmask_fake)
            loss_g = 0.5 * loss_g_style + 0.5 * loss_g_global
