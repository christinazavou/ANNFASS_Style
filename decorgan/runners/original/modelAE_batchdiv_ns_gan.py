...
class IM_AE(IM_AE_ORIG_COMMON):
    ...
    def generator_step(self, content_batch, style_batch, store_grads=False):
        ...
        for (coarse_content, mask_content, Dmask_content), \
            ...
            loss_g_global = -torch.sum(torch.log(d_out_style + 1e-8) * Dmask_fake) * self.param_alpha / torch.sum(Dmask_fake)
            loss_g_style = -torch.sum(torch.log(d_out_global + 1e-8) * Dmask_fake) / torch.sum(Dmask_fake)
