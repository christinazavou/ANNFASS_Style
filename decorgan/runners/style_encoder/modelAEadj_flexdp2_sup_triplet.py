...
class IM_AE(IM_AE_ADJ_COMMON):
    ...
    def __init__(self, config):
        ...
        self.train_triplet_loader = get_loader(config.data_dir,
                                               os.path.join(config.triplet_splits, f"train_triplets_{config.split}{config.pct_suffix}.txt"),
                                               batch_size=8)
        self.test_triplet_loader = get_loader(config.data_dir,
                                              os.path.join(config.triplet_splits, f"test_triplets_{config.split}.txt"),
                                              batch_size=8)
    ...
    def train(self, config):
        ...
        while epoch < training_epoch:
            ...
            triplet_losses = []
            for batch_idx, batch in enumerate(iter(self.train_triplet_loader)):

                if batch_idx > 0 and config.debug:
                    break
                ...
                for anchor_path, pos_path, neg_path in zip(*batch):
                    ...
                    anchor_voxel_style = torch.from_numpy(anchor_voxel_style).to(self.device).unsqueeze(
                        0).unsqueeze(0)
                    ...
                    D_out_anchor = self.discriminator.layer(anchor_voxel_style)
                    D_out_pos = self.discriminator.layer(pos_voxel_style)
                    D_out_neg = self.discriminator.layer(neg_voxel_style)
                    ...
                    loss = triplet_criterion(dist_anchor_pos, dist_anchor_neg, target)
