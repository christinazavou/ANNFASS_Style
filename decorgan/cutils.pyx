...
@cython.boundscheck(False)
@cython.wraparound(False)
def get_patches_edge_dilated(char[:, :, ::1] img,
                             char[:, :, ::1] edge,
                             char[:, :, ::1] dilated,
                             char[:, :, :, ::1] patches,
                             char[:, :, :, ::1] patches_edge,
                             char[:, :, :, ::1] patches_dilated,
                             int patch_size):
    cdef int dimx,dimy,dimz
    cdef int margin_size
    cdef int p,i,j,k
    ...
    for i in range(margin_size,dimx-margin_size-1):
        for j in range(margin_size,dimy-margin_size-1):
            for k in range(margin_size,dimz-margin_size-1):
                # Christina Zavou: i think the following finds patches where at least one voxel is full and one
                # is empty so that the patch is interesting; i dont see any difference between patches, patches_edge
                # and patches_dilated
                if (img[i,j,k]==0 or img[i,j,k+1]==0 or img[i,j+1,k]==0 or
                    ...
                    img[i+1,j+1,k]==0 or img[i+1,j+1,k+1]==0) and \
                        (img[i,j,k]==1 or img[i,j,k+1]==1 or img[i,j+1,k]==1 or
                         img[i,j+1,k+1]==1 or img[i+1,j,k]==1 or img[i+1,j,k+1]==1 or
                         img[i+1,j+1,k]==1 or img[i+1,j+1,k+1]==1):
                    patches[p] = img[i-margin_size:i+margin_size+2,
                                     j-margin_size:j+margin_size+2,
                                     k-margin_size:k+margin_size+2]
                    patches_edge[p] = edge[i-margin_size:i+margin_size+2,
                                           j-margin_size:j+margin_size+2,
                                           k-margin_size:k+margin_size+2]
                    patches_dilated[p] = dilated[i-margin_size:i+margin_size+2,
                                                 j-margin_size:j+margin_size+2,
                                                 k-margin_size:k+margin_size+2]
                    p += 1
    # Christina Zavou: As i understand it patches should be a large tensor (big first dimension)
    # where many places at the end can remain empty (zeros). this is why we return p (maximum number of patches made)
    return p
...
@cython.boundscheck(False)
@cython.wraparound(False)
def get_useful_random_patches(float[:, :, :] img,
                              float[:, :, :, :] patches,
                              int patch_size,
                              int patch_num,
                              int[:, :] i_j_k):
...
@cython.boundscheck(False)
@cython.wraparound(False)
def get_useful_random_paired_patches(char[:, :, :] img,
                                     float[:, :, :] img2,
                                     char[:, :, :, :] patches,
                                     float[:, :, :, :] patches2,
                                     int patch_size,
    ...
    assert img.shape[0] == img2.shape[0] and img.shape[1] == img2.shape[1] and img.shape[2] == img2.shape[2]
        ...
        while not found:
            ...
            if (img[i,j,k]==0 or img[i,j,k+1]==0 or img[i,j+1,k]==0 or
                ...
                patches[p] = img[i-margin_size:i+margin_size+2,
                                 j-margin_size:j+margin_size+2,
                                 k-margin_size:k+margin_size+2]
                patches2[p] = img2[i-margin_size:i+margin_size+2,
                                 j-margin_size:j+margin_size+2,
                                 k-margin_size:k+margin_size+2]
                i_j_k[p, 0] = i
                i_j_k[p, 1] = j
