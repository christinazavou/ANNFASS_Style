...
for f_or_d in os.listdir(source_root):
    if os.path.isdir(os.path.join(source_root, f_or_d)):
        for f_or_d_2 in os.listdir(os.path.join(source_root, f_or_d)):
            if os.path.isfile(os.path.join(source_root, f_or_d, f_or_d_2)) and f_or_d_2 == "model.obj":
                this_name = os.path.join(source_root, f_or_d, f_or_d_2)
                out_name = os.path.join(target_dir, f_or_d, "model.obj")
                run(this_name, out_name)
            else:
                this_name = os.path.join(source_root, f_or_d, f_or_d_2, "model.obj")
                assert os.path.exists(this_name)
                out_name = os.path.join(target_dir, f_or_d, f_or_d_2, "model.obj")
                run(this_name, out_name)
