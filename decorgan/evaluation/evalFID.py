...
def compute_FID_for_real(config):
    ...
    for content_id in range(dataset_len):
        ...
        if config.output_size==128:
            tmp_raw = get_vox_from_binvox_1over2(os.path.join(data_dir,dataset_names[content_id]+"/model_depth_fusion.binvox")).astype(np.uint8)
        elif config.output_size==256:
    ...
    for style_id in range(max_num_of_styles):
        ...
        if config.output_size==128:
            tmp_raw = get_vox_from_binvox_1over2(os.path.join(data_dir,styleset_names[style_id]+"/model_depth_fusion.binvox")).astype(np.uint8)
        elif config.output_size==256:
