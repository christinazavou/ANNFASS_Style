...
def eval_IOU(device, style_set, dset, config):
    ...
    #load style shapes
    styleset_len = min(len(style_set), max_num_of_styles)

    #load content shapes
    dataset_len = min(len(dset), max_num_of_contents)
    ...
    for content_id in range(dataset_len):
        ...
        for style_id in range(styleset_len):
            ...
            output_shape_coarse_tensor = F.max_pool3d(output_shape_tensor, kernel_size = dset.upsample_rate, stride = dset.upsample_rate, padding = 0)
...
def precompute_unique_patches_per_style(device, styleset, config):
    ...
    patches_dir = config.unique_patches_dir
    if not os.path.exists(patches_dir):
        os.makedirs(patches_dir)
    else:
        if len(os.listdir(patches_dir)) > 0:
            print("won't override")
            return


    #load style shapes
    styleset_len = len(styleset)
    print("precompute_unique_patches:",config.stylepath,styleset_len)
...
def eval_LP_Div_IOU(device, style_set, dset, config):
    # result_dir = "output_for_eval"
    result_dir = config.output_for_eval_dir
    if not os.path.exists(result_dir):
        print("ERROR: result_dir does not exist! "+result_dir)
    ...
    #prepare dictionary for style shapes
    dict_style_patches_tensor = []
    for style_id in range(len(style_set)):
        data_dict = h5py.File(patches_dir+"/style_"+str(style_id)+".hdf5", 'r')
        patches = data_dict['patches'][:]
        data_dict.close()
    ...
    for content_id in range(dataset_len):
        for style_id in range(styleset_len):
            ...
            #IOU
            similar_flags = np.zeros([patch_num,len(style_set)], np.int32)
            for patch_id in range(patch_num):
                for compare_id in range(len(style_set)):
...
def eval_LP_Div_Fscore(device, style_set, dset, config):
    # result_dir = "output_for_eval"
    result_dir = config.output_for_eval_dir
    if not os.path.exists(result_dir):
        print("ERROR: result_dir does not exist! "+result_dir)
