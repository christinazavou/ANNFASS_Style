...
class ShapeNet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if self.voxel_size==128:
            tmp_raw = get_vox_from_binvox_1over2(self.img_names[index]).astype(np.float32)
...
def train_classifier(device, config):
...
def compute_FID_for_real(device, style_set, dset, config):

    class_num = 24

    styleset_len = len(style_set)
    ...
    out_file = config.cls_dir+"/precomputed_real_mu_sigma_"+str(voxel_size)+"_max_content_"+str(max_num_of_contents)+"_num_style_"+str(max_num_of_styles)+".hdf5"
    if os.path.exists(out_file):
        print("won't override")
        return

    Clsshapenet = classifier(32, z_dim, class_num, voxel_size)
    Clsshapenet.to(device)
    Clsshapenet.load_state_dict(torch.load(config.cls_dir+'/Clsshapenet_'+str(voxel_size)+'.pth'))
    ...
    for content_id in range(dataset_len):
        ...
        if config.output_size==128:
            tmp_raw = get_vox_from_binvox_1over2(dset.files[content_id]).astype(np.uint8)
        elif config.output_size==256:
            tmp_raw = get_vox_from_binvox(dset.files[content_id]).astype(np.uint8)
    ...
    for style_id in range(max_num_of_styles):
        ...
        if config.output_size==128:
            tmp_raw = get_vox_from_binvox_1over2(style_set.files[style_id]).astype(np.uint8)
        elif config.output_size==256:
            tmp_raw = get_vox_from_binvox(style_set.files[style_id]).astype(np.uint8)
    ...
    hdf5_file = h5py.File(config.cls_dir+"/precomputed_real_mu_sigma_"+str(voxel_size)+"_max_content_"+str(max_num_of_contents)+"_num_style_"+str(max_num_of_styles)+".hdf5", mode='w')
...
def eval_FID(device, config):
    ...
    hdf5_file = h5py.File(config.cls_dir+"/precomputed_real_mu_sigma_"+str(voxel_size)+"_max_content_"+str(max_num_of_contents)+"_num_style_"+str(max_num_of_styles)+".hdf5", mode='r')
